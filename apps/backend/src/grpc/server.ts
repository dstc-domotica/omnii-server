 ...
 */
async function upsertSystemInfo(
	instanceId: string,
	systemInfo: {
		supervisor: string;
...
const omniiServiceImpl: IOmniiServiceServer = {
	...
	enroll: async (
		...
		try {
			...
			console.log(
				`[gRPC] Enrollment successful. Instance ID: ${result.instanceId}`,
			);
		} catch (error: unknown) {
			const message =
				error instanceof Error ? error.message : "Enrollment failed";
			console.log(`[gRPC] Enrollment failed: ${message}`);
	...
	handshake: async (
		...
		console.log(
			`[gRPC] Handshake successful for addon_id: ${addonId}, session_id: ${sessionId.substring(0, 8)}...`,
		);
	...
	heartbeat: async (
		...
		if (!session) {
			console.log(
				`[gRPC] Heartbeat from unknown session: ${sessionId.substring(0, 8)}...`,
			);
		...
		if (systemInfo) {
			...
			console.log(
				`[gRPC] Updated system info for instance: ${session.instanceId}`,
			);
		}
	...
	reportUpdates: async (
		...
		if (!session) {
			console.log(
				`[gRPC] Update report from unknown session: ${sessionId.substring(0, 8)}...`,
			);
		...
		console.log(
			`[gRPC] Updated reported updates for instance: ${session.instanceId} (${components.length} components)`,
		);
	...
	triggerUpdate: async (
		...
		console.log(
			`[gRPC] TriggerUpdate request - session: ${sessionId.substring(0, 8)}..., type: ${updateType}, addon: ${addonSlug}`,
		);
